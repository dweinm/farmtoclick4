...
class PermitVerification(Document):
    meta = {'collection': 'permit_verifications', 'indexes': ['user', 'status', 'created_at']}
    
    user = ReferenceField(User, required=True)
    status = StringField(required=True, default='pending', choices=['pending', 'verified', 'rejected', 'under_review'])
