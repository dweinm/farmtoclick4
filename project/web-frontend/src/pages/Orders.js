              ...
              <button className="user-profile-btn" onClick={handleProfileDropdown}>
                <div className="user-avatar">
                  {user.profile_picture ? (
                    <img src={`/uploads/profiles/${user.profile_picture}`} alt={user.first_name} style={{ width: '100%', height: '100%', objectFit: 'cover', borderRadius: '50%' }} onError={(e) => { e.target.style.display = 'none'; e.target.nextElementSibling.style.display = 'inline-block'; }} />
                  ) : null}
                  <i className="fas fa-user" style={{ display: user.profile_picture ? 'none' : 'inline-block' }}></i>
                </div>
                ...
                return (
                  <article key={orderId} className={`order-card order-collapsible ${isExpanded ? 'is-open' : ''}`}>
                    <div className="order-header">
                      ...
                      <button className="order-toggle" type="button" aria-expanded={isExpanded} onClick={() => toggleOrder(orderId)}>
                        <span>Details</span>
